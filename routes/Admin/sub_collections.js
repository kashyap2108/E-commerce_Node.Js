...
router.get("/", (req, res) => {
  ...
  SubCollections.find()
    ...
    .catch(err =>
      res.status(404).json({ subcollections: "There are no subcollections for given collection!!" })
    );
});
...
router.post(
  ...
  (req, res) => {
    ...
    sub_collection_name = req.body.sub_collection_name ? req.body.sub_collection_name : "";
